...
void isr() //interrupt service routine for IR reflective encoder
{
  rev++;
}
...
void setup()
{
  ...
  /* initialize IR reflective pins */
  pinMode(FRONT_ENCODER_PIN,INPUT);//定义该红外接口为输入接口

  /* attach interrupt to IR reflective pins */
  ...
  Serial.println("r: IR reflective sensor output continously");
...
void loop()
{
  ...
  else if (serialData == 'L') /* amplifier uses channel B with gain 128 to amplify the signal for Arduino */
  /* Add while loop to continuously output data*/
  {
    while (1)
    {
  ...
  //  }
    ...
    while (1)
    {
      ...
      interval=millis()-oldtime; //finds the time
      rpm=(rev/interval)*60000/17; //calculates rpm for blades
